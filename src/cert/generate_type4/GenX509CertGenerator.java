 ...
 * 
 */
public class GenX509CertGenerator {
    /** 提供强加密随机数生成器 (RNG)* */
    private SecureRandom sr;
 
    public GenX509CertGenerator() throws NoSuchAlgorithmException,NoSuchProviderException {
	...
	public void createCert(X509Certificate certificate, PrivateKey rootPrivKey,KeyPair kp,CertParamEntity childCaParam) throws CertificateException, IOException,
        ...
        // 2253 style).
        X500Name subject = new X500Name(childCaParam.getDnName());
...
//        AlgorithmId algorithmid = signer.getAlgorithmId();
        ...
        Date edate = new Date();
        // 天 小时 分 秒 毫秒
        edate.setTime(bdate.getTime() + childCaParam.getValidDay() * 24L * 60L * 60L * 1000L);
...
//        base64.encodeBuffer(x509certimpl1.getEncoded(), fos);
      //新的
        ...
        String encoded = encoder.encode(x509certimpl1.getEncoded());  
        FileWriter fw = new FileWriter(childCaParam.getCaPath());  
        ...
        try {
            ...
            savePfx(childCaParam.getCaAlias(), kp.getPrivate(), childCaParam.getCaKeyStorePwd(), certChain,childCaParam.getCaKeyStorePath());
            FileInputStream in = new FileInputStream(childCaParam.getCaKeyStorePath());
...
//         outputKeyStore.setEntry("scriptx", pke, password);
	...
	public void createRootCA(CertParamEntity param) throws NoSuchAlgorithmException,
        ...
        // "CN=Dave, OU=JavaSoft, O=Sun Microsystems, C=US". (RFC 1779 or RFC
        // 2253 style)
        X500Name subject = new X500Name(param.getDnName());
        ...
        try {
            savePfx(param.getCaAlias(), cak.getPrivateKey(), param.getCaKeyStorePwd(), certs,param.getCaKeyStorePath());
        } catch (Exception e) {
            e.printStackTrace();
        }
//        //下面这种方式保存到文件里没有开始结束标志
//        BASE64Encoder base64 = new BASE64Encoder();
//        FileOutputStream fos = new FileOutputStream(new File(param.getCaPath()));
//        // fos.write(certificate.getEncoded());
//        // 生成（保存）cert文件 base64加密 当然也可以不加密,不加密就是上面的方法
//        base64.encodeBuffer(certificate.getEncoded(), fos);
//        fos.close();
        
        BASE64Encoder encoder = new BASE64Encoder();  
        String encoded = encoder.encode(certificate.getEncoded());  
        FileWriter fw = new FileWriter(param.getCaPath());  
        fw.write("-----BEGIN CERTIFICATE-----\r\n");    //非必须  
        fw.write(encoded);  
        fw.write("\r\n-----END CERTIFICATE-----");  //非必须  
        fw.close();  
    ...
    public void signCert(CertParamEntity childCaParam) throws NoSuchAlgorithmException,CertificateException, IOException, UnrecoverableKeyException,
        ...
        try {
        	CertParamEntity rootCaParam = childCaParam.getParentCa();
            KeyStore ks = KeyStore.getInstance("pkcs12");
            FileInputStream ksfis = new FileInputStream(rootCaParam.getCaKeyStorePath());
 
            char[] storePwd = rootCaParam.getCaKeyStorePwd().toCharArray();
            char[] keyPwd = rootCaParam.getCaKeyStorePwd().toCharArray();
            // 从给定输入流中加载此 KeyStore。
            ...
            PrivateKey privK = (PrivateKey) ks.getKey(rootCaParam.getCaAlias(), keyPwd);
